...
**无感记账** 是一款极简快捷的记账应用。通过深度集成 iOS 系统特性（App Intents + 快捷指令 + iPhone 15 Pro 操作按钮），支付完成后只需按一下按钮，即可完成记账。

### ✨ 核心特性

- ⚡ **极速记账**: 支付完成 → 按下操作按钮 → 自动记录（2 秒完成）
- 🎯 **一键配置**: App 内一键下载快捷指令，无需手动配置
- 🤖 **智能分类**: AI 自动推断消费类型，越用越准确
- 🔒 **隐私安全**: 所有处理在本地完成，截图用后即删
- 📊 **数据洞察**: 清晰了解消费习惯和趋势
...
1. 启动 App 并完成引导流程
2. 进入设置页面，点击"一键添加快捷指令"（推荐）
   - 或查看"手动配置引导"自行创建
3. **仅 iPhone 15 Pro**: 前往 设置 → 操作按钮 → 选择刚添加的快捷指令
   - **其他机型**: 可通过 Siri、小组件或轻点背面触发
4. 进行一次小额支付测试
5. 在支付结果页面按下操作按钮（或使用其他触发方式）
6. 返回 App 查看自动记账结果

> 详细配置教程请查看 [快捷指令配置指南](docs/SHORTCUT_SETUP.md)
